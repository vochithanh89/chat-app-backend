...
class MessageService {
  ...
  async createMessage(input: CreateMessageInput): Promise<Message> {
    ...
    realtime.emitToConversation(conversationId, 'message:new', this.serialize(message))
  ...
  async forward(messageId: number, userId: number, conversationIds: number[]): Promise<Message[]> {
    ...
    for (const convId of conversationIds) {
      ...
      realtime.emitToConversation(convId, 'message:new', this.serialize(m))
      created.push(m)
    }
    return created
  }

  serialize(m: Message): Record<string, unknown> {
    return {
      id: m.id,
      ...
      sender: (m as any).sender
        ? {
            id: (m as any).sender.id,
            name: (m as any).sender.name,
            avatarUrl: (m as any).sender.avatarUrl,
          }
        : undefined,
